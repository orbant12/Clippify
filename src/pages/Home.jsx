...
//UPDATES DEPENDING ON USER "FILE-Storage" DOCS.__________________________________________
useEffect(() => {
  if (!currentuser) {
    setFolders([]);
    return;
...
//RECENTLY ADDED____________________________//
useEffect(() => {
...
return (
                  ...
                  <div className='usage-box'>
                    {/*TITLE*/}
                      ...
                      <div className='follow-row'>
                       ...
                       <a style={{color:"black"}} href="https://www.instagram.com/clippify.app/"> <div className='socialMedia'>
                          <InstagramIcon />
                        <h3>Instagram</h3>
                        </div></a>

                        <a style={{color:"black"}} href="https://www.tiktok.com/@clippify.app"><div className='socialMedia'>
                        <ScreenshotIcon />
                        <h3>Tiktok</h3>
                        </div></a>
    ...
    <div className="no-document">No Recent Clips's added</div>
          ...
          <Link to={`/folder/${recentFiles.folder_id}/${recentFiles.id}`}>
              ...
              <FileCard imgSrc={recentFiles.img} imgAlt="file img" title={recentFiles.title} tags={recentFiles.tag} related_count={recentFiles.related_count} video_size={recentFiles.video_size}/>
              ...
              {/*ADDED DOM*/}
              {folders.length === 0 ? (
    <div className="no-folder"><a href="/memory" className='no-folder-a' >No Folders Yet <br /><div className='inline_txt'><CreateNewFolderIcon sx={{mt:3,pt:0}}/></div></a></div>
  ) : (
                  folders.map((folder) => 
                    folder && folder.id ? (
